...
class Criminal:
    def __init__(self,root):
        ...
        self.var_arrest_data=StringVar()
        ...
        lbl_title=Label(self.root,text="CRIMINAL MANAGEMENT SYSTEM SOFTWARE",font=('times new roman',40,'bold'),bg='black',fg='brown')
        ...
        img1=Image.open('images/img2.jpg')
        ...
        img_2=Image.open('images/img1.jpg')
        ...
        img_3=Image.open('images/img7.jpg')
        ...
        txt_arrestDate=ttk.Entry(upper_frame,textvariable=self.var_arrest_data,width=22,font=('arial',11,"bold"))
        ...
        lbl_occuoution=Label(upper_frame,font=('arial',12,'bold'),text="Occuoution:",bg='white')
        lbl_occuoution.grid(row=4,column=0,sticky=W,padx=2,pady=7)

        txt_occuoution=ttk.Entry(upper_frame,textvariable=self.var_occupation,width=22,font=('arial',11,"bold"))
        txt_occuoution.grid(row=4,column=1,padx=2,pady=7)
        ...
        btn_add=Button(button_frame,command=self.add_data,text='Record Save',font=('arial',13,'bold'),width=14,bg='chartreuse',fg='black')
        btn_add.grid(row=0,column=0,padx=3,pady=5)

        #Update buttone

        btn_update=Button(button_frame,command=self.update_data,text='Update',font=('arial',13,'bold'),width=14,bg='chartreuse',fg='black')
        btn_update.grid(row=0,column=1,padx=3,pady=5)

        #Delete buttone

        btn_delete=Button(button_frame,command=self.delete_data,text='Delete',font=('arial',13,'bold'),width=14,bg='chartreuse',fg='black')
        btn_delete.grid(row=0,column=2,padx=3,pady=5)

        #Clear buttone

        btn_clear=Button(button_frame,command=self.clear_data,text='Clear',font=('arial',13,'bold'),width=14,bg='chartreuse',fg='black')
        btn_clear.grid(row=0,column=3,padx=3,pady=5)

        #background right side image

        img_crime=Image.open('images/img8.jpg')
        ...
        search_by=Label(search_frame,font=('arial',11,'bold'),text="Search By",bg='palegreen',fg='black')
        ...
        btn_search=Button(search_frame,command=self.search_data,text='Search',font=('arial',13,'bold'),width=14,bg='chartreuse',fg='black')
        btn_search.grid(row=0,column=3,padx=3,pady=5)

        #All buttone

        btn_all=Button(search_frame,command=self.fetch_data,text='Show All',font=('arial',13,'bold'),width=14,bg='chartreuse',fg='black')
        btn_all.grid(row=0,column=4,padx=3,pady=5)



        crimeagency=Label(search_frame,font=('Perpetua Titling MT',27,'bold'),text="NATIONAL CRIME AGENCY",bg='white',fg='Dark orange')
        crimeagency.grid(row=0,column=5,sticky=W,padx=40,pady=0)
    ...
    def add_data(self):
        if self.var_case_id.get()=="":
            ...
            try:
                ...
                self.clear_data()
                conn.close()
                messagebox.showinfo('Success','Criminal recode has been added')
    ...
    def get_cursor(self,event=" "):
        ...
        self.var_arrest_data.set(data[4])
    ...
    def update_data(self):
        if self.var_case_id.get()=="":
            ...
            try:
                update=messagebox.askyesno('Update','Are you sure update this criminal records')
                if update>0:
                    conn=mysql.connector.connect(host='localhost',username='root',password='Hyperlink@06',database='management')
                    my_cursor=conn.cursor()
                    my_cursor.execute('update criminal set Criminal_no=%s,Criminal_name=%s,Nick_name=%s,arrest_date=%s,dateOfcrime=%s,address=%s,age=%s,occupation=%s,BirthMark=%s,crimeType=%s,fatherName=%s,gender=%s,wnated=%s where Case_id=%s',(
                        self.var_criminal_no.get(),
                        self.var_name.get(),
                        self.var_nickname.get(),
                        self.var_arrest_data.get(),
                ...
                self.clear_data()
                conn.close()
                messagebox.showinfo('Success','Criminal recodes has been successfully updated')
            except Exception as es:
                messagebox.showerror('Error',f'Due to{str(es)}')            
    ...
    def delete_data(self):
        if self.var_case_id.get()=="":
            ...
            try:
                Delete=messagebox.askyesno('Delete','Are you sure update this criminal records')
                if Delete>0:
                    conn=mysql.connector.connect(host='localhost',username='root',password='Hyperlink@06',database='management')
                    my_cursor=conn.cursor()
                ...
                self.clear_data()
                conn.close()
                messagebox.showinfo('Success','Criminal recodes has been successfully Deleted')
            except Exception as es:
                messagebox.showerror('Error',f'Due to{str(es)}')    
